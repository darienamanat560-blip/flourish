...
export const MARKERS = {
  ...
  hdl: {
    display: "HDL",
    aliases: ["hdl", "hdl cholesterol", "hdl-c", "hdl cholesterol direct", "high density lipoprotein"],
    unit: "mg/dL",
  ...
  ldl: {
    display: "LDL",
    aliases: ["ldl", "ldl cholesterol", "ldl-c", "ldl chol calc", "ldl cholesterol calc", "low density lipoprotein"],
    unit: "mg/dL",
...
export function findCanonical(name) {
  ...
  const stripped = norm
    .replace(/\b(serum|plasma|level|levels|count|test|reading|value)\b/g, "")
    .trim()
    .replace(/\s+/g, " ");
  if (ALIAS_TO_CANONICAL[stripped]) return ALIAS_TO_CANONICAL[stripped];

  // Substring matches as last resort
