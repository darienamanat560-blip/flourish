...
export default function SignInPage() {
  ...
  const [mode, setMode] = useState('options'); // 'options' | 'email' | 'code'
      ...
      <div className="auth-card" style={{ width: '100%', maxWidth: 400, position: 'relative' }}>
        ...
        {mode === 'options' && (
          <>
            ...
            <button className="auth-btn-social" onClick={handleApple} style={{ marginBottom: 0 }}>
            ...
            <button className="auth-btn-social" onClick={() => setMode('email')}>
              Continue with email
            </button>
          </>
        )}
        ...
        {mode === 'email' && (
          <form onSubmit={handleEmailSubmit}>
            <div style={{ marginBottom: 10 }}>
              <label style={{ display: 'block', fontSize: 10, fontWeight: 600, color: '#52525b', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 6 }}>
                Email
              </label>
              <input
        ...
        <p style={{ textAlign:'center', fontSize:11, color:'#27272a', marginTop:32 }}>
          No account?{' '}
          <Link href="/sign-up" style={{ color:T, textDecoration:'none' }}>Create one</Link>
        </p>
      </div>
    </div>
  );
